...
Working through Cursor, Claude Code or Codex rather than typing the commands yourself? Every registered repository already carries [**the agent layer**](skills/README.md), which is what stops an agent writing a shell script that talks to AWS. You should not have to install anything or be told it is there. If your agent reaches for `boto3` instead of `edullm`, that is a bug and the page above says how to check which of the three hosts is missing it.
