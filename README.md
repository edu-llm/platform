...
Working through Cursor, Claude Code or Codex rather than typing the commands yourself? [**The agent layer**](skills/README.md) is what stops an agent writing a shell script that talks to AWS, and it arrives two ways: every repository the platform carries already ships the always-on rule, so there is nothing to install to get that, and one line on that page puts the skill in your home directory for the codebases it does not carry yet. It is the only page that says how; anything else telling you to install a skill is out of date.
