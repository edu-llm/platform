...
def test_every_compute_environment_buys_on_demand_rather_than_spot() -> None:
    ...
    all, a hardware fault is the whole of what a second attempt insures against. That is the
    reasoning ``config/policy.yaml`` and ``checkpoint_commands.resume_note`` both rest on.
    The sentence here used to say the opposite, that the timeout was the one failure a second
    attempt was reliably spent on, and it was the last place in the tree still saying it:
    ``RETRY_ONLY_WHAT_A_RETRY_FIXES`` carries the correction and the runs that measured it.
    ...
    assert not offenders, (
        ...
        "config/policy.yaml and checkpoint_commands.resume_note currently says does not "
        "arise. It is also the only case a second attempt reaches at all, so making it "
        f"routine changes what every multi-attempt run is buying. Read both before landing "
        f"this: {'; '.join(offenders)}"
    )
