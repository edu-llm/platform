...
RESULT_MANIFEST_DIGEST = "sha256:48af8b390f1aec167356b5f822dbf7b9af025a522181883166a436fe2ffc80e4"
...
def test_success_marker_must_live_inside_its_own_checkpoint(
    ...
    with pytest.raises(ValidationError) as exc_info:
        CheckpointManifest.model_validate(checkpoint_payload(success_marker_uri=success_marker_uri))
...
def test_result_manifest_rejects_two_checkpoints_at_the_same_step() -> None:
    with pytest.raises(ValidationError) as exc_info:
        ResultManifest.model_validate(
            result_payload(checkpoints=[checkpoint_payload(), later_checkpoint_payload(step=1000)])
        )
    assert_validation_error(
        exc_info.value,
        error_type="value_error",
