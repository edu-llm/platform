...
from edullm_platform.operational_inventory import (
    ...
    InventoryCheck,
    OperationalInventoryReport,
    evaluate_operational_inventory,
...
def get_check(result: OperationalInventoryReport, check_id: str) -> InventoryCheck:
...
def test_the_shipped_configuration_passes_every_inventory_check() -> None:
    result = evaluate_operational_inventory(loaded_inputs())
...
def test_the_checks_pass_when_the_github_plan_supports_private_repo_controls() -> None:
...
def test_a_passing_check_reports_the_ok_reason_code(check_id: str) -> None:
    result = evaluate_operational_inventory(loaded_inputs())
...
def test_ownership_fails_for_unexpected_admin_roster() -> None:
    ...
    result = evaluate_operational_inventory(replace(inputs, inventory=inventory))
...
def test_workload_coverage_fails_without_gpu_representative() -> None:
    ...
    result = evaluate_operational_inventory(replace(inputs, catalog=catalog))
...
def test_approval_paths_fails_when_denial_paths_incomplete() -> None:
    ...
    result = evaluate_operational_inventory(replace(inputs, policy=policy))
...
def test_representative_manifests_fails_for_unregistered_compute_profile() -> None:
    ...
    result = evaluate_operational_inventory(replace(inputs, manifests=manifests))
...
def test_representative_manifests_fails_when_the_registry_stops_listing_their_dataset() -> None:
    ...
    result = evaluate_operational_inventory(replace(inputs, dataset_registry=successor_only))
    check = get_check(result, "inventory_representative_manifests")
    assert check.passed is False
    assert check.reason_code == "unregistered_dataset", (
...
def test_loading_the_inputs_rejects_invalid_organization_yaml(tmp_path: Path) -> None:
    repo_root = copy_inventory_repo(tmp_path)
    (repo_root / "config" / "organization.yaml").write_text(
        "admins: []\nteam_leads: []\nmembers: []\npilot_repositories: []\n",
        encoding="utf-8",
    )
    with pytest.raises(ValidationError) as exc_info:
        load_inventory_inputs(repo_root)
    too_short_locations = {
...
def test_aws_capacity_fails_when_node_count_requires_more_vcpus_than_quota() -> None:
    ...
    result = evaluate_operational_inventory(replace(inputs, catalog=scaled_catalog))
    check = get_check(result, "inventory_aws_capacity")
    assert check.passed is False
    assert check.reason_code == "capacity_increase_required"
...
def test_the_report_round_trips_through_contract_json() -> None:
    ...
    round_tripped = OperationalInventoryReport.model_validate(payload)
...
def test_evidence_stale_fails_github_plan_and_aws_capacity_checks(tmp_path: Path) -> None:
    ...
    for filename in ("github-plan.sanitized.json", "service-quotas.sanitized.json"):
        ...
        evidence_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    inputs = load_inventory_inputs(repo_root)
    ...
    result = evaluate_operational_inventory(inputs)
