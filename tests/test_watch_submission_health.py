...
def test_a_job_after_the_compile_job_is_never_a_judgement() -> None:
    ...
    Asked in the wrong order this lands as unreadable, which would leave an admission
    ...
    fault = classify_failure(
        ...
        created_at="2026-08-06T05:45:32Z",
        failing_jobs=["Submit the approved manifest to admission"],
        log=None,
    )
    assert fault.kind == "platform_fault"
    assert "admission" in fault.reason
...
def test_the_watcher_asks_for_no_more_than_it_needs() -> None:
    ...
    assert permissions == {"contents": "read", "actions": "read", "issues": "write"}
