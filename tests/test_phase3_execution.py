...
def test_no_rule_names_the_timeout_because_a_terminated_job_never_reaches_the_rules() -> None:
    ...
    ``checkpoint_commands.resume_note`` and ``config/policy.yaml`` reached the same
    conclusion from the first of the runs below and now say so; this is the third place, and
    the three no longer disagree. What follows from it is that the second attempt insures
    against a lost host and nothing else, so
    ``test_every_compute_environment_buys_on_demand_rather_than_spot`` stops being a
    preference about purchasing and becomes the pin under the only arm that can fire.
