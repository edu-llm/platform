...
**The workload list is the sharp one.** ``dolma-tokenize-smoke`` is a registered workload
naming a repository that nothing registers -- there is no ECR repository for dolma and no
image can be published for it. It compiles, classifies as routine, routes to a lead, and is
refused at admission with ``unregistered_repository`` *after* the approval. It is
deliberately absent from the dropdown, and the test below is what makes that absence a
decision rather than an oversight: the moment dolma is registered, the option has to appear.
"""
...
def test_the_workload_dropdown_offers_only_workloads_whose_repository_is_registered() -> None:
    ...
    """
    catalog = registry("workload-catalog.yaml")["workloads"]
    registered = {entry["repository"] for entry in registry("repositories.yaml")["repositories"]}
    runnable = sorted(
        workload["name"] for workload in catalog if workload["repository"] in registered
    )

    assert options_for("workload_profile") == runnable
