...
def test_the_count_rule_can_see_a_count_that_has_moved() -> None:
    ...
    # ``edullm stop`` made it ten and ``edullm data`` made it eleven, and being made to edit
    # it is the point rather than the friction: a control sentence that stayed correct through
    # a change of the count would be asserting that the rule ignores this phrase, which is the
