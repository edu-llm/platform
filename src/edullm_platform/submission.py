...
def compile_submission(
    ...
    *,
    ...
    # OTHER END OF THE SAME SHELF. The six above refuse a command that would waste a
    # machine, lose its state, sell a retry nothing has earned, mean something it does not,
    # or not run at all. This refuses one that would run, fail, and be written down as
    # having worked -- because a shell exits with the status of the last thing it ran, and
    # nothing between the form and Batch required that to be the program.
    #
    # Last of the seven, and it is the only one that takes the command and nothing else. It
