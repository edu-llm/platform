...
Verification run: 426 tests executed, 426 passed, 0 failed, 0 errored, pytest exit code 0.
...
- The one criterion of this phase that does not block a pilot, and the reason is what it asks for. Every other check in the list is a refusal, and a refusal that does not happen is money, data, attribution or lineage lost. This one asks that a difference between two builds be explainable, which is what a reviewer needs in order to accept the gate and is not what stands between a pilot user and harm: an unexplained difference in an image config costs somebody an afternoon, and the digest pinning that makes the image trustworthy is proved elsewhere in this list rather than here.
