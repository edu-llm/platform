...
def _log_lines(log_stream: str, *, profile: str, region: str) -> list[str]:
    ...
    per step -- runs to thousands of lines, so a window taken from the head reaches its
    limit somewhere inside the model and reports a run that printed nothing.
    """
    events = aws_json(
        [
            "logs", "get-log-events",
...
def _summary_object(lines: Sequence[str]) -> Mapping[str, Any] | None:
    ...
    the lines before it are not all prose: a config dump and a W&B teardown each put a bare
