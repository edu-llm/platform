...
"find the four places", and the finding is what took the afternoon. This writes all three.
...
def plan(root: Path, entry: RegisteredRepository, reason: str) -> list[Edit]:
...
def verify(edits: Sequence[Edit], entry: RegisteredRepository) -> None:
    ...
    Failing here leaves the tree untouched, because all three files are written together
...
def pull_request_body(entry: RegisteredRepository, reason: str, *, base_known: bool) -> str:
    """What a reviewer needs that the diff does not say, and nothing the diff already says.

    The follow-up list is the part worth having. Three of its five steps are not
    predictable from the three files this changes, so a reviewer who approves the diff and
    stops has left a registration that looks complete and cannot publish.
    """
    lines = [
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        edits = plan(root, entry, str(options.reason))
        verify(edits, entry)
        ...
        json.dumps(
            {
                ...
                "pull_request_body": pull_request_body(entry, reason, base_known=base_known),
