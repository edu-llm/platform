...
# THE TOKENIZER MAP MOVED TO `edullm_platform.tokenizers` AND IS NOT IMPORTED BACK. It was
# declared here and read only from outside, which was tenable while a maintainer's generator
# was the only place it could plausibly live. `edullm data` reads it now, on a laptop, where
# no `tools/` directory exists -- so it had to be inside the package, and a re-export here
# would be a second name for a map whose whole purpose is to have one. That module carries
# the entries and every measurement behind each of them.
